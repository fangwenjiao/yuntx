...
public class GroupSqlManager extends AbstractSQLManager {
    ...
    public static Cursor getGroupCursor() {
        try {
            String sql = "select groupid, name, type, count ,permission ,joined from groups where joined =1 order by joined desc , create_date desc";
    ...
    public static List<ECGroup> getJoinGroups() {
        ...
        try {
            String sql = "select groupid, name from groups where joined=1 order by create_date desc";;
    ...
    public static boolean getJoinState(String groupId) {
        String sql = "select groupid, joined from groups where groupId = '" + groupId + "'";
        Cursor cursor = getInstance().sqliteDB().rawQuery(sql, null);
        if(cursor != null && cursor.getCount() > 0) {
    ...
    public static long insertGroup(ECGroup group , boolean join , boolean ignoreJoin) {
        ...
        try {
            ...
            if(isExitGroup(group.getGroupId())) {
                return getInstance().sqliteDB().update(DatabaseHelper.TABLES_NAME_GROUPS, values, "groupid = ?", new String[]{group.getGroupId()});
            }
            long rowId = getInstance().sqliteDB().insert(DatabaseHelper.TABLES_NAME_GROUPS, null, values);
            return rowId;
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public static int updateJoinStatus(String groupId , boolean join) {
        try {
            ...
            return getInstance().sqliteDB().update(DatabaseHelper.TABLES_NAME_GROUPS, values, "groupid = ?", new String[]{groupId});
        } catch (Exception e) {
            e.printStackTrace();
        }
        return -1;
    ...
    public static int delGroup(String groupId) {
        try {
            return getInstance().sqliteDB().delete(DatabaseHelper.TABLES_NAME_GROUPS, "groupid = ?", new String[]{groupId});
        } catch (Exception e) {
            e.printStackTrace();
        }
        return -1;
    ...
    public static int delALLGroup() {
        try {
            return getInstance().sqliteDB().delete(DatabaseHelper.TABLES_NAME_GROUPS, null, null);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return -1;
    ...
    public static List<String> getAllGroupIdBy(boolean joined) {
        ...
        Cursor cursor = null;
        try {
            String sql = "select groupid from groups where joined = " + (joined?1:0);
            cursor = getInstance().sqliteDB().rawQuery(sql, null);
            if(cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
                    String groupid = cursor.getString(0);
    ...
    public static ECGroup getECGroup(String groupId) {
        try {
            String sql = "select name, type, count ,permission ,joined ,declared ,owner from groups where groupid = '" + groupId + "'";
            Cursor cursor = getInstance().sqliteDB().rawQuery(sql, null);
            if(cursor != null && cursor.getCount() > 0) {
                cursor.moveToFirst();
                ECGroup group = new ECGroup();
    ...
    public static boolean isNeedApply(String groupId) {
        boolean joind = false;
        try {
            String sql = "select joined from groups where groupid = '" + groupId + "'";
            Cursor cursor = getInstance().sqliteDB().rawQuery(sql, null);
            if(cursor != null && cursor.getCount() > 0) {
                cursor.moveToFirst();
