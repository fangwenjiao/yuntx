...
public class ConversationSqlManager extends AbstractSQLManager {
    ...
    public static Cursor getConversationCursor() {
        try {
            ...
            String sql = "SELECT unreadCount, im_thread.type, sendStatus, dateTime, sessionId, text, username ,name \n" +
                    "      FROM im_thread \n" +
                    "      LEFT JOIN contacts ON im_thread.sessionId = contacts.contact_id \n" +
                    "      LEFT JOIN groups ON im_thread.sessionId = groups.groupid order by dateTime desc;";
