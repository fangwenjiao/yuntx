...
public class GroupInfoActivity extends ECSuperActivity implements
  ...
  /*      if(members != null) {
            Collections.sort(members , new Comparator<ECGroupMember>() {
                @Override
                public int compare(ECGroupMember lhs, ECGroupMember rhs) {
                    return Integer.valueOf(lhs.getRole()).compareTo(Integer.valueOf(rhs.getRole()));
                }
            });
        }*/
        if(members == null) {
        ...
        mGroupCountTv.setText(getString(R.string.str_group_members_tips, isOwner()? mAdapter.getCount() - 1 :mAdapter.getCount()));
